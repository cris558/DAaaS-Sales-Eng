...
dbutils.fs.ls("abfss://pie-eip-p@stpdmfdiiun01sa.dfs.core.windows.net/CrisHudici")

# COMMAND ----------

df = spark.read.option("header",True).csv("abfss://pie-eip-p@stpdmfdiiun01sa.dfs.core.windows.net/CrisHudici/toronto_departures.csv")
display(df)
...
df.createOrReplaceTempView('toronto_departures')     # saved in a hadoop table (view)
